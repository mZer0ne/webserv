...
export default function DatabaseView() {
    ...
    const selectContainer = async (c: DbContainer) => {
        ...
        if (c.engine !== 'mysql' && c.engine !== 'postgres') {
            setError(`Browsing not supported for ${c.engine} yet.`);
            return;
        }
        try {
        ...
        <div style={{display: 'grid', gridTemplateColumns: '260px 1fr', gap: 16, height: '100%'}}>
            <div style={{display: 'flex', flexDirection: 'column', gap: 16}}>
                <div style={card}>
                    ...
                    {containers.length === 0 &&
                        <span style={{color: 'var(--text-muted)', fontSize: '0.85rem'}}>No DB containers found.</span>}
                    {containers.map((c) => (
                        <div key={c.id} onClick={() => selectContainer(c)} style={{
                            ...
                            <div style={{fontWeight: 600, fontSize: '0.9rem'}}>
                                <span className={`dot ${c.state === 'running' ? 'active' : ''}`}
                                      style={{marginRight: 6}}/>
                                {c.name}
                            </div>
          ...
          <textarea value={sql} onChange={(e) => setSql(e.target.value)}
                    ...
                    <div style={{marginTop: 10, display: 'flex', gap: 10, alignItems: 'center'}}>
                        ...
                        {activeDb && <span
                            style={{color: 'var(--text-muted)', fontSize: '0.8rem'}}>against <b>{activeDb}</b></span>}
                    </div>
                </div>
                        ...
                        <table className="project-table">
                            ...
                            {result.rows.map((row, ri) => (
                                <tr key={ri}>{row.map((cell, ci) => (
                                    <td key={ci}
                                        style={{fontFamily: 'var(--font-mono)', fontSize: '0.8rem'}}>{cell}</td>
                                ))}</tr>
                            ))}
                            </tbody>
                        </table>
