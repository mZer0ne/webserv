    ...
    const renderRow = (s: ServiceInfo, project: string | null) => {
        ...
        return (
            <tr key={s.id}>
                <td style={{
                    ...td,
                    paddingLeft: project ? 46 : 20,
                    borderLeft: `3px solid ${running ? stripe : 'transparent'}`
                }}>{display}</td>
                <td style={{
                    ...td,
                ...
                <td style={td}>
                    {running ? (
                        <button className="btn-action stop" title="Stop"
                                onClick={() => control(s.id, 'stop')}>■</button>
                    ) : (
                        <button className="btn-action start" title="Start"
                                onClick={() => control(s.id, 'start')}>▶</button>
                    )}
                    <button className="btn-action" title="Restart" disabled={!running}
                            onClick={() => control(s.id, 'restart')}>↻
                    </button>
            ...
            <div>
                ...
                <div className="project-table" style={{display: 'block', padding: 0}}>
                    ...
                    {overviewServices.length === 0 ? (
                        <div style={{padding: 24, textAlign: 'center', color: 'var(--text-muted)'}}>No services
                            running.</div>
                    ) : (
                            ...
                            {[...projectGroups.entries()].map(([project, members]) => {
                                ...
                                return (
                                    <React.Fragment key={project}>
                                        <tr style={{background: 'rgba(255,255,255,0.02)', cursor: 'pointer'}}
                                            ...
                                            <td style={td} onClick={(e) => e.stopPropagation()}>
                                                {anyRunning ? (
                                                    <button className="btn-action stop" title="Stop all"
                                                            onClick={() => groupControl(members, 'stop')}>■</button>
                                                ) : (
                                                    <button className="btn-action start" title="Start all"
                                                            onClick={() => groupControl(members, 'start')}>▶</button>
                                                )}
                                                <button className="btn-action" title="Restart all"
                                                        onClick={() => groupControl(members, 'restart')}>↻
                                                </button>
                                            </td>
                                        </tr>
            ...
            <div>
                ...
                <div className="metric-panel">
                    ...
                    <div className="metric-row">
                        <span>Pressure {metrics?.memory.pressure ?? 0} %</span><span>{metrics?.memory.usedGB ?? 0}/{metrics?.memory.totalGB ?? 0} GB</span>
                    </div>
                    ...
                    <div className="metric-sub">
                        ...
                        <div>Free<b>{((metrics?.memory.totalGB ?? 0) - (metrics?.memory.usedGB ?? 0)).toFixed(1)} GB</b>
                        </div>
                    </div>
                </div>
                ...
                <div className="metric-panel">
                    ...
                    <div className="metric-row">
                        <span>{metrics?.storage.percent ?? 0} %</span><span>{fmtBytes(metrics?.storage.usedBytes ?? 0)} / {fmtBytes(metrics?.storage.totalBytes ?? 0)}</span>
                    </div>
                </div>
...
function Gauge({value}: { value: number }) {
    ...
    return (
        <svg viewBox="0 0 200 110" style={{width: '100%'}}>
            ...
            <text x={cx} y={cy - 18} textAnchor="middle" fontSize="20" fontWeight="700"
                  fill="var(--text-primary)">{v}%
            </text>
        </svg>
    );
}
