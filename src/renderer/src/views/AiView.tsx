        ...
        <div>
            ...
            <div style={{...card, marginBottom: 16}}>
                <div style={{display: 'flex', gap: 10, alignItems: 'center'}}>
                    ...
                    <button className="btn-primary" disabled={!!pulling || !pullName.trim()}
                            onClick={() => pull(pullName)}>
                        {pulling ? `Pulling ${pulling}…` : '⬇ Pull'}
                    </button>
                </div>
                <div style={{marginTop: 12, display: 'flex', gap: 8, flexWrap: 'wrap'}}>
                    {SUGGESTED.map((m) => (
                        <button key={m} className="stack-badge"
                                style={{cursor: 'pointer', background: 'rgba(255,255,255,0.04)'}}
                                disabled={!!pulling} onClick={() => pull(m)}>+ {m}</button>
                    ))}
                </div>
                {pulling &&
                    <p style={{marginTop: 10, color: 'var(--text-muted)', fontSize: '0.82rem'}}>Downloading {pulling} —
            ...
            {/* Installed models */}
            {models.length === 0 ? (
                <div style={{...card, textAlign: 'center', color: 'var(--text-muted)', marginBottom: 16}}>No models yet.
                    Pull one above.</div>
            ) : (
                <table className="project-table" style={{marginBottom: 16}}>
                    ...
                    {models.map((m) => (
                        <tr key={m.name}>
                            ...
                            <td>
                                <button className="btn-action" title="Test" onClick={() => setTestModel(m.name)}>💬
                                </button>
                                <button className="btn-action stop" title="Delete" onClick={() => remove(m.name)}>🗑
                                </button>
                            </td>
                        </tr>
                    ))}
            ...
            <div style={card}>
                ...
                <div style={{display: 'flex', gap: 10, marginBottom: 10}}>
                    <select style={{...inp, maxWidth: 240}} value={testModel}
                    ...
                    <button className="btn-primary" disabled={thinking || !testModel || !prompt.trim()}
                            onClick={runTest}>
                        {thinking ? '…' : 'Send'}
                    </button>
                </div>
