...
export default function SitesView() {
  ...
  const add = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) { setForm(EMPTY); await refresh(); }
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      if (res.success) { setEditing(null); await refresh(); }
  ...
  const startWeb = async () => {
    setBusy(true); setError('');
    try { await window.api.sites.ensureWeb(); await refresh(); }
    catch (e: any) { setError(e.message); }
    finally { setBusy(false); }
  };
  ...
  return (
    <div>
      ...
      <div style={{ ...card, marginBottom: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 20px' }}>
        <div style={{ fontSize: '0.85rem' }}>
          ...
          {!ca?.trusted && <span style={{ color: 'var(--text-muted)' }}> — install it so browsers trust your site certificates.</span>}
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          {ca?.generated && <button className="btn-action" title="Reveal CA file" onClick={() => window.api.tls.revealCa()}>📂</button>}
          {!ca?.trusted && <button className="btn-primary" disabled={busy} onClick={installCa}>Install root CA</button>}
        </div>
      </div>

      <form onSubmit={add} style={{ ...card, marginBottom: 20, display: 'grid', gridTemplateColumns: '0.9fr 1.3fr 1.6fr 1fr auto', gap: 12, alignItems: 'end' }}>
        {typeField(form, setForm)}
        <Field label="Domain">
          <input required placeholder={form.type === 'proxy' ? 'api.test' : 'blog.test'} style={inp} value={form.domain}
          ...
          <>
            <Field label="Document root">
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button type="button" className="btn-action" title="Choose folder" onClick={() => pick(setForm, true)} style={{ flexShrink: 0 }}>📁</button>
              </div>
            </Field>
      ...
      {sites.length === 0 ? (
        ...
        <table className="project-table">
          <thead><tr><th>Domain</th><th>Type</th><th>Target / Root</th><th>URL</th><th>Actions</th></tr></thead>
          <tbody>
            {sites.map((s) => (
              <tr key={s.id}>
                ...
                <td style={{ whiteSpace: 'nowrap' }}>
                  <a className="domain-link" href={`https://${s.domain}:${httpsPort}`} target="_blank" rel="noreferrer">
                    🔒 {s.domain}:{httpsPort}
                  </a>
      ...
      {editing && (
        <div className="modal-overlay" onClick={() => setEditing(null)}>
          <div className="modal-box" style={{ maxWidth: 560 }} onClick={(e) => e.stopPropagation()}>
            <header><span style={{ fontWeight: 600 }}>Edit site · {editing.domain}</span><button className="btn-action" onClick={() => setEditing(null)}>✕</button></header>
            <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>
              {typeField(editForm, setEditForm)}
              <Field label="Domain">
                <input style={inp} value={editForm.domain} onChange={(e) => setEditForm({ ...editForm, domain: e.target.value })} />
              </Field>
              {editForm.type === 'proxy' ? (
                <>
                ...
                <>
                  <Field label="Document root">
                    <div style={{ display: 'flex', gap: 8 }}>
                      <input style={inp} value={editForm.root} onChange={(e) => setEditForm({ ...editForm, root: e.target.value })} />
                      <button type="button" className="btn-action" title="Choose folder" onClick={() => pick(setEditForm, false)} style={{ flexShrink: 0 }}>📁</button>
                    </div>
                  </Field>
                  <Field label="PHP version">
                    <select style={inp} value={editForm.php} onChange={(e) => setEditForm({ ...editForm, php: e.target.value })}>
                      <option value="">Static (no PHP)</option>
                      {phpVersions.map((v) => <option key={v} value={v}>PHP {v}</option>)}
                    </select>
                  </Field>
              ...
              {error && <div style={{ color: 'var(--color-danger)', fontFamily: 'var(--font-mono)', fontSize: '0.82rem' }}>{error}</div>}
