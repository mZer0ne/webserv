...
export default function RuntimesView() {
        ...
        return (
            <tr key={r.id}>
                ...
                <td style={{...td, textAlign: 'center'}}>
                    {r.installed ? (
                        <label className="toggle"><input type="checkbox" checked={r.running} disabled={working}
                                                         onChange={toggle}/><span className="track"/></label>
                    ) : <span style={{color: 'var(--text-muted)'}}>—</span>}
                </td>
                <td style={{...td, textAlign: 'right', whiteSpace: 'nowrap'}}>
                    <button className="btn-action" title="Settings" disabled={working}
                            onClick={() => openConfig(r.id, r.label, r.internalPort)}>⚙️
                    </button>
                    {r.category === 'PHP' && (
                        <button className="btn-action" title="Edit php.ini" disabled={working}
                                onClick={() => openIni(r.id, r.label)}>📝</button>
                    )}
                    {!r.installed ? (
                        <button className="btn-action" title="Install" disabled={working} onClick={toggle}>⬇</button>
                    ) : (
                        <>
                            <button className="btn-action" title="Restart" disabled={working || !r.running}
                                    onClick={() => act(r.id, () => window.api.services.control(r.containerId!, 'restart'))}>🔄
                            </button>
                            <button className="btn-action" title="Logs" disabled={!r.running}
                                    onClick={() => showLogs(r.label, r.containerId)}>📄
                            </button>
                            <button className="btn-action stop" title="Uninstall" disabled={working} onClick={() => {
                                if (confirm(`Remove ${r.label}?`)) act(r.id, () => window.api.runtimes.uninstall(r.id));
                            }}>🗑
                            </button>
                        </>
                    )}
    ...
    const familyRow = (f: DbFamilyStatus) => {
        ...
        return (
            <tr key={f.id}>
                ...
                <td style={{...td, textAlign: 'center'}}>
                    {vs.installed ? (
                        <label className="toggle"><input type="checkbox" checked={vs.running} disabled={working}
                                                         onChange={toggle}/><span className="track"/></label>
                    ) : <span style={{color: 'var(--text-muted)'}}>—</span>}
                </td>
                <td style={{...td, textAlign: 'right', whiteSpace: 'nowrap'}}>
                    <button className="btn-action" title="Settings" disabled={working}
                            onClick={() => openConfig(rowId, `${f.label} ${sel}`, f.internalPort)}>⚙️
                    </button>
                    {!vs.installed ? (
                        <button className="btn-action" title={`Install ${f.label} ${sel}`} disabled={working}
                                onClick={toggle}>⬇</button>
                    ) : (
                        <>
                            <button className="btn-action" title="Restart" disabled={working || !vs.running}
                                    onClick={() => act(rowId, () => window.api.services.control(vs.containerId!, 'restart'))}>🔄
                            </button>
                            <button className="btn-action" title="Logs" disabled={!vs.running}
                                    onClick={() => showLogs(`${f.label} ${sel}`, vs.containerId)}>📄
                            </button>
                            <button className="btn-action stop" title="Uninstall" disabled={working} onClick={() => {
                                if (confirm(`Remove ${f.label} ${sel}?`)) act(rowId, () => window.api.runtimes.uninstall(rowId));
                            }}>🗑
                            </button>
                        </>
                    )}
    ...
    const webServerRow = () => {
        ...
        const wrap = (p: Promise<unknown>) => p.then(() => ({success: true})).catch((e: any) => ({
            success: false,
            error: e.message
        }));
        ...
        return (
            <tr key="web">
                ...
                <td style={{...td, ...mono}}>—</td>
                <td style={{...td, textAlign: 'center'}}>
                    <label className="toggle"><input type="checkbox" checked={running} disabled={working}
                                                     onChange={toggle}/><span className="track"/></label>
                </td>
                <td style={{...td, textAlign: 'right', whiteSpace: 'nowrap'}}>
                    <button className="btn-action" title="Restart / apply" disabled={working}
                            onClick={() => act('web', () => wrap(window.api.sites.ensureWeb()))}>🔄
                    </button>
                    {installed && (
                        <button className="btn-action stop" title="Remove web server" disabled={working}
                                onClick={() => {
                                    if (confirm('Remove the web server container?')) act('web', () => wrap(window.api.sites.removeWeb()));
                                }}>🗑</button>
                    )}
                </td>
            </tr>
            ...
            {logs && (
                <div className="modal-overlay" onClick={() => setLogs(null)}>
                    <div className="modal-box" onClick={(e) => e.stopPropagation()}>
                        <header><span style={{fontWeight: 600}}>Logs · {logs.name}</span>
            ...
            {cfg && (
                <div className="modal-overlay" onClick={() => setCfg(null)}>
                    <div className="modal-box" style={{maxWidth: 520}} onClick={(e) => e.stopPropagation()}>
                        <header><span style={{fontWeight: 600}}>Settings · {cfg.label}</span>
                            <button className="ctrl-btn" onClick={() => setCfg(null)}>✕</button>
                        </header>
                        <div style={{padding: 20, display: 'flex', flexDirection: 'column', gap: 16}}>
                            <div>
                                <label style={{display: 'block', marginBottom: 6, fontSize: '0.85rem'}}>
                                    Host port <span
                                    style={{color: 'var(--text-muted)'}}>→ container :{cfg.internalPort} (blank = not published)</span>
                                </label>
                                <input type="number" value={cfgPort} placeholder={String(cfg.internalPort)}
                                       onChange={(e) => setCfgPort(e.target.value)}
                                       style={selStyle as React.CSSProperties}/>
                            </div>
                            <div>
                                <label style={{display: 'block', marginBottom: 6, fontSize: '0.85rem'}}>
                                ...
                                <textarea value={cfgEnv} onChange={(e) => setCfgEnv(e.target.value)} rows={5}
                                          ...
                                          style={{
                                              ...selStyle as React.CSSProperties,
                                              width: '100%',
                                              resize: 'vertical'
                                          }}/>
                            </div>
            ...
            {ini && (
                <div className="modal-overlay" onClick={() => setIni(null)}>
                    <div className="modal-box" style={{maxWidth: 760}} onClick={(e) => e.stopPropagation()}>
                        <header><span style={{fontWeight: 600}}>php.ini · {ini.label}</span>
                            <button className="ctrl-btn" onClick={() => setIni(null)}>✕</button>
                        </header>
              ...
              <textarea value={iniText} onChange={(e) => setIniText(e.target.value)} spellCheck={false} rows={20}
                        style={{
                            ...
                            <div style={{fontSize: '0.78rem', color: 'var(--text-muted)'}}>
                                Overlaid on the image defaults (mounted at conf.d). Saving restarts this PHP-FPM
                                service.
                            </div>
                            <div style={{display: 'flex', gap: 10, justifyContent: 'flex-end'}}>
